...
func TestMountSpec_Validate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mount: MountSpec{HostPath: "/Users/dev/code/app", GuestPath: "/Users/dev/code/app", Writable: true},
		},
		{
			name:  "distinct guest path, as WSL produces",
			mount: MountSpec{ProjectID: "3fa9c2b1d0", HostPath: "/Users/dev/code/app", GuestPath: "/mnt/avr/projects/3fa9c2b1d0", Writable: true},
		},
		{name: "read-only is a legal mapping", mount: MountSpec{HostPath: "/a", GuestPath: "/a"}},
		{name: "no host path", mount: MountSpec{GuestPath: "/a"}, wantErr: true},
		{name: "no guest path", mount: MountSpec{HostPath: "/a"}, wantErr: true},
		{name: "relative host path", mount: MountSpec{HostPath: "code/app", GuestPath: "/a"}, wantErr: true},
		{name: "unnormalized host path", mount: MountSpec{HostPath: "/a/../a", GuestPath: "/a"}, wantErr: true},
		{name: "relative guest path", mount: MountSpec{HostPath: "/a", GuestPath: "mnt/avr"}, wantErr: true},
		{name: "unnormalized guest path", mount: MountSpec{HostPath: "/a", GuestPath: "/mnt/avr/../avr"}, wantErr: true},
		{name: "trailing separator is not normalized", mount: MountSpec{HostPath: "/a", GuestPath: "/mnt/avr/"}, wantErr: true},
	}
...
func TestMountSpec_GuestPathIsJudgedAsALinuxPath_REQ_18_5(t *testing.T) {
	...
	if err := (MountSpec{HostPath: hostAbs(t), GuestPath: "/mnt/avr/projects/abc"}).Validate(); err != nil {
		t.Errorf("an absolute Linux guest path was rejected: %v", err)
	}
	if err := (MountSpec{HostPath: hostAbs(t), GuestPath: `C:\Users\ola\code\app`}).Validate(); err == nil {
...
func TestNormalizeMounts_CanonicalisesAndOrdersTheSet(t *testing.T) {
	...
	got, err := NormalizeMounts([]MountSpec{
		{HostPath: "/b/../b/two/", GuestPath: "/b/two/", Writable: true},
		{HostPath: "/a/one", GuestPath: "/a/one", Writable: true},
		{HostPath: "/a/one", GuestPath: "/a/one", Writable: true},
	})
	if err != nil {
	...
	want := []MountSpec{
		{HostPath: "/a/one", GuestPath: "/a/one", Writable: true},
		{HostPath: "/b/two", GuestPath: "/b/two", Writable: true},
	}
...
func TestNormalizeMounts_RefusesASetThatContradictsItself_PROP_5(t *testing.T) {
	...
	_, err := NormalizeMounts([]MountSpec{
		{HostPath: "/a", GuestPath: "/mnt/avr/projects/x", Writable: true},
		{HostPath: "/b", GuestPath: "/mnt/avr/projects/x", Writable: true},
	})
	if err == nil {
	...
	if _, err := NormalizeMounts([]MountSpec{
		{HostPath: "/a", GuestPath: "/one", Writable: true},
		{HostPath: "/a", GuestPath: "/two", Writable: true},
	}); err == nil {
...
func TestMountHostPaths_KeepsOrder(t *testing.T) {
	...
	got := MountHostPaths([]MountSpec{
		{HostPath: "/a", GuestPath: "/mnt/a"},
		{HostPath: "/b", GuestPath: "/mnt/b"},
	})
...
// hostAbs returns an absolute path in the host's own vocabulary, so the test
// says something about guest-path handling rather than about the host's.
func hostAbs(t *testing.T) string {
	t.Helper()
	return t.TempDir()
}
