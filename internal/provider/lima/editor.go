...
func (p *Provider) EditorTarget(ctx context.Context, machine, guestPath string) (provider.EditorTarget, error) {
	...
	authority := "ssh-remote+avr-" + machine

	// Replace the Lima host name with avar's remote authority, leaving
	// every other directive — Hostname, Port, IdentityFile, User — intact
	// so that the resulting stanza is self-contained and needs no Lima
	// knowledge to use.
