...
| Native workspace requested on a backend that has none | `provider.NativeWorkspacer` type assertion in `cmd/` | Say the environment reaches the project directly and that the flag is unnecessary; do not fail as though something broke (Req 14.1, Req 17.3). |
