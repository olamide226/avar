...
- **Distribution**: GoReleaser → GitHub Releases; Homebrew tap (`brew install --cask olamide226/tap/avar`, cask depends on `lima`) for macOS; checksummed `avr.exe` zip archives for Windows, also submitted to winget as `olamide226.avar` (a portable package, through a pull request to `microsoft/winget-pkgs` from a fork) with no WSL dependency declared, because WSL setup stays behind avar's own explained first-run offer (Req 18.3, 18.15). Both hosts offer the command under `avr` and `avar` (Req 18.17): the macOS cask links the one executable twice (`binary "avr", target: "avar"`), and the Windows archive ships a second copy of the binary as `avar.exe`, which is what gives winget a third `NestedInstallerFiles` entry *and* gives the command to somebody who only unzips the archive. `avr` stays canonical in help, errors and documentation. *Considered and rejected: a second `NestedInstallerFiles` entry pointing at `avr.exe` with `PortableCommandAlias: avar`.* The 1.12.0 schema permits it (no `uniqueItems`, nothing requiring distinct `RelativeFilePath`), but winget's validation — which the `microsoft/winget-pkgs` pipeline runs — keys a set on `ToLower(RelativeFilePath)` and raises `DuplicateRelativeFilePath` ("Duplicate relative file path found.") at error level (`ManifestValidation.cpp`, verified against current `master`). Writing the second entry as `.\avr.exe` evades the literal string comparison and has merged precedent (`Fly-io.flyctl`, 19 versions), but it depends on an open bug (winget-cli #2887); GoReleaser cannot express it either, its winget pipe emitting one entry per archive binary with no field to add another. And it would do nothing for the archive download, where the file is the command.
...
The operations are **segregated by capability** rather than gathered into one interface. `Provider` is the core set every backend must implement; `Snapshotter`, `EditorTargetProvider`, `PortDiagnoser`, `EditorProber`, `MountLimiter`, `MachineSizer`, and `SSHAgentForwarder` describe optional abilities. Callers type-assert for a capability and report plainly when it is absent. Editor launch is modeled by a transport-neutral target, so WSL is not forced through SSH.
...
// EditorProber reports the editor windows connected to a running machine, which
// avar's session records cannot see (Req 5.10, §3.8). It is read-only, it never
// starts a stopped machine to ask, a stopped machine is an empty answer, and an
// error means "could not find out", never "none" (Req 5.11).
...
type EditorConnection struct {
    Editor string // "VS Code", "Cursor", "Zed"
    PID    int    // the guest process that shows the window: an extension host, Zed's proxy
}
...
| Editor probe | `ConnectedEditors` runs the shared editor probe (`internal/provider/editors`) unprivileged through `limactl shell`, in the same base64 form as the listener probe, and only on an instance `limactl list` reports running. Verified against Lima 2.2.0 with real VS Code 1.135.0 and Zed 1.20.2 servers in an Ubuntu 24.04 guest (§3.8). |
...
| Editor probe | Run the shared editor probe (`internal/provider/editors`) as root through `wsl.exe --exec`, as the listener probe runs, and only on a distribution the invocation's listing reports running: `wsl --exec` starts a stopped distribution, and a probe that started the machine it is deciding about would undo the stop. **Not verified on a real WSL host.** VS Code's WSL layout (`~/.vscode-server/bin/<commit>`) and Zed's launch through `wsl.exe --cd ~ --exec env .zed_server/… proxy` are taken from the editors' own sources (§3.8). |
...
- **Amended 2026-09-22 (Req 18.17): an alias registers as the program it aliases.** avar is installed under two names on both hosts, and `os.Executable` reports the one the user typed — on macOS it returns the cask's symlink rather than its target (measured with go1.26), and on Windows `avar.exe` is a second real file. Every registration is keyed by that path (the plist's `ProgramArguments`, the Windows stamp), so alternating between the names had each invocation take the other's registration for one to repair: on macOS a plist rewrite plus `launchctl list`/`unload`/`load`, on Windows `schtasks /Query` and `/Create /F` — subprocesses on the path Req 17.1 budgets at 500 ms, on every alternation. Nothing was ever duplicated (both hosts key the entry by a fixed label, `com.avar.idle-check` and `avar-idle-check`), and nothing settled either. `canonicalBinary` maps an alias name to the canonical binary beside it before anything is recorded; a lone `avar.exe` with no `avr.exe` beside it registers as itself, which is stable too. It costs one map lookup on the warm path, and a stat only for a run started under an alias.

#### Editors connected without a session (Req 5.10, 5.11)

*Added after implementation.* `avr code`, `avr cursor` and `avr zed` launch the editor and exit. The editor then reaches the guest on its own — over SSH on Lima, through its WSL integration on Windows — and runs a remote server there that avar never started and never records. The idle check read only avar's session records, so somebody working only in their editor had the environment stopped under them once the timeout passed, on both hosts.

**Detection rule.** A guest process is a connected editor window when the program it runs lies inside the editor's server directory (matched as a path component, because Zed runs its server by a relative path) *and* it is the process that exists once per connected window:

| Editor | Program | Sign of a connected window | Evidence |
|---|---|---|---|
| VS Code | `…/.vscode-server/…/node` | `--type=extensionHost` among its arguments: the remote extension host, one per window | **Measured.** Real VS Code 1.135.0 server (commit 08d4889f) in a Lima 2.2.0 Ubuntu 24.04 guest, installed where Remote-SSH 0.128.0's `linux-exec-server-installer.sh` puts it (`~/.vscode-server/code-<commit> command-shell --cli-data-dir ~/.vscode-server/cli`, server under `cli/servers/Stable-<commit>/server`), with a real window (headless Chrome on the server's workbench). The extension host appeared when the window attached as `<server>/node --dns-result-order=ipv4first <server>/out/bootstrap-fork --type=extensionHost --transformURIs --useHostProxy=false`; exited at once when the window closed ("The client has disconnected gracefully"); and stayed when the connection was cut ("will wait for reconnection 3h before disposing", default 10800000 ms). The directory name is VS Code's `product.json` `serverDataFolderName`. |
| Cursor | `…/.cursor-server/…/node` | `--type=extensionHost` | **Constructed.** Cursor is a VS Code fork with its own server directory; not installed on the capture host. |
| Zed | `.zed_server/zed-remote-server-<channel>-<version>` | first argument `proxy`: what Zed's client runs per connection, and which exits with it | **Source and measured.** Zed v1.20.2's `crates/remote/src/transport/ssh.rs` and `wsl.rs` both install under `paths::remote_server_dir_relative()` (`.zed_server` — on WSL too; the `.zed_wsl_server` that `paths.rs` declares is not used by either transport) and run `env <relative path> proxy --identifier <id>` from `~` (`cd; …` over SSH, `wsl.exe --cd ~ --exec env …`). A real 1.20.2 server whose proxy was started that way shows the proxy as `.zed_server/zed-remote-server-stable-1.20.2 proxy --identifier …`, relative, and the daemon it spawns with an absolute path. |

*The server alone does not count.* After the last window has gone, VS Code's server stays five minutes, and only when started with `--enable-remote-auto-shutdown` (the ServerLifetime shutdown timer, `300*1e3` in 1.135.0's `server-main.js`; Remote-SSH passes the flag), and Zed's daemon ten (`IDLE_TIMEOUT` in `crates/remote_server/src/server.rs`). A server started without its auto-shutdown would keep a machine up for good. Remote-SSH's `command-shell` is not used either: it outlives a dropped SSH connection until its installer's keep-alive loop next fails to write (up to 180 s), and adds nothing while an extension host is present.

*A dropped window counts.* A VS Code window whose connection dropped without closing — a laptop lid, a lost network — keeps its extension host for the reconnection grace time, three hours by default, so that it can reconnect with its terminals and state. That is the session Property 11 protects, so the environment stays for as long as VS Code keeps it. The cost is bounded by the grace time.

**Where it lives.** `provider.EditorProber` (§3.4), an optional capability both backends implement through one shared guest probe, `internal/provider/editors`, on the pattern of `internal/provider/listeners`: `Script` lists `/proc/[0-9]*/cmdline` using only `sh` and `tr`, and `Parse` decides in Go, against captured output, and returns `[]provider.EditorConnection`. The command layer names no backend (`cmd/purity_test.go`). The probe's output includes every process's command line, which can carry secrets (VS Code's server takes `--connection-token=…`); it is parsed in memory and never logged, and only the editor's name and the process id leave the backend.

**What gets probed.** Only a machine the idle check is about to stop: recorded, no live session, idle clock past the timeout, and running according to `Status`. A stopped machine in the idle set is converged by `Stop` without being probed; a machine with a session or time left on its clock is not probed. The check makes at most one round trip into each machine it is about to stop, and nothing on avar's warm path changes.

**Probe failure.** The machine is left running and its idle clock unchanged; the other idle machines are still stopped; the check exits 1 with an error naming the environment and the cause, which the host scheduler records. The next check asks again. A backend that does not implement `EditorProber` is decided on session records alone.

**Idle clock.** An editor window found restarts the machine's idle clock (`session.RestartIdleClock`), so the machine gets a full Idle_Timeout from the last check that saw a window rather than being stopped at the first check after the window closes.

**`avr code` at launch: yes, it restarts the idle clock,** immediately before launching the editor, in the shared flow `avr cursor` and `avr zed` use too. The check can see an editor only once its server is installed and a window has connected, which on a first connection includes downloading the server, and a check that ran in that gap would stop the environment the user had just asked to open. The cost is one small file write on the editor path, not on the shell's warm path. A failure to write it is printed as a warning and does not stop the editor opening.
...
_For any_ machine with at least one live session — an avar session, or a connected window of an editor avar opens — the idle agent SHALL NOT stop it, regardless of elapsed time; and _for any_ running machine for which the backend could not report whether an editor window is connected, the idle agent SHALL NOT stop it on that check.
**Validates: 5.5, 5.10, 5.11**

*Extended after implementation.* The property quantified over avar sessions, and `avr code`, `avr cursor` and `avr zed` hold none: the editor they open works in the guest through its own server. A machine somebody was editing in was therefore outside the property and was stopped under them, with the property green (see docs/lessons.md, "A property that quantifies over part of the design gives false confidence"). The second clause is the direction the property takes when the evidence is missing: not knowing is not "idle".
...
| Editor probe fails during the idle check (transport down, guest refuses the script) | `EditorProber.ConnectedEditors` returns an error | Leave that environment running and its idle clock unchanged; stop the other idle environments; exit 1 with an error naming the environment and the cause, so the host scheduler records a failed run. The next check asks again. Never read as "no editor" (Req 5.11, Property 11). |
| Idle clock cannot be restarted | `session.RestartIdleClock` returns an error | In the idle check: keep the environment its editor holds, and exit 1 naming it. In `avr code`, `avr cursor` and `avr zed`: print a warning naming the environment and saying to run the command again if it stops before the editor connects, and open the editor anyway (Req 5.10). |
